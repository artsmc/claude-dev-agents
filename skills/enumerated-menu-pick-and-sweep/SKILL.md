...
His picks often carry a second clause bolted onto the chosen item — a follow-on action or an inline answer: `option 1 and update envs when your done`, `1. yes, and execute it`. Treat the rider as part of the same instruction: do the selected option AND the appended directive in one move. The rider is scoped to that item — `update envs when your done` is the tail of choosing option 1, not a separate request to negotiate.
...
Mark frequently writes his own `#`-numbered list of decisions to apply across a spec sheet or codebase — e.g. `#1 in your spec sheet draw a mermaid uml of the schema. #2 same for the api. #4 the OTP from sentient-monorepo is the pattern we should match. #8 instead of linear we will use sentient-monrepo`. Each numbered line is a directive; sweep them top to bottom and apply every one. His numbering can skip (`#1 #2 #4 #8`) — do not invent or fill the gaps, just resolve the items he actually wrote. Each `#N` is independent: some are tasks, some are source-of-truth declarations, some are scope cuts. Apply them all; do not collapse them into a vague summary.
...
## Red flags
The full table of misreads and Mark's corrections (re-asking after a bare pick, dropping a rider, treating an in-sweep question as resolved, re-pitching a cut item, filling skip-numbered gaps, over-building a demoted item), plus the verbatim pick examples, lives in `references/examples.md` — read it when a terse reply maps ambiguously onto your menu.
