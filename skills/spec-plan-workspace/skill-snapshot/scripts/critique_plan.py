...
Usage:
    python critique_plan.py /path/to/feature-folder [--focus area1,area2] [--tier quick|standard|full]

Tier behavior:
    quick    - Only critiques task-list.md
    standard - Critiques FRD.md, TR.md, task-list.md (no FRS/GS)
    full     - Critiques all 5 files (FRD, FRS, GS, TR, task-list)

When --tier is omitted, auto-detects based on which files exist in docs/.
...
class SpecCritic:
    ...
    def __init__(self, feature_path: Path, focus_areas: List[str] = None, tier: str = None):
        self.feature_path = feature_path
        self.focus_areas = focus_areas or []
        self.tier = tier or self._detect_tier()
    ...
    def _detect_tier(self) -> str:
        """Auto-detect tier based on which files exist in docs/."""
        docs_dir = self.feature_path / "docs"
        if not docs_dir.exists():
            return 'quick'

        has_frd = (docs_dir / "FRD.md").exists()
        has_frs = (docs_dir / "FRS.md").exists()
        has_gs = (docs_dir / "GS.md").exists()
        has_tr = (docs_dir / "TR.md").exists()
        has_tasks = (docs_dir / "task-list.md").exists()

        if has_frd and has_frs and has_gs and has_tr and has_tasks:
            return 'full'
        if has_frd and has_tr and has_tasks:
            return 'standard'
        return 'quick'

    def critique(self) -> Dict:
        """Run tier-appropriate critique checks."""
        # Always run scope appropriateness check
        self._critique_scope_appropriateness()

        if self.tier == 'quick':
            # Quick: only task breakdown
            self._critique_task_breakdown()
        elif self.tier == 'standard':
            # Standard: FRD requirements (no FRS), technical design, task breakdown
    ...
    def _critique_scope_appropriateness(self):
        """Check if the chosen tier matches the actual scope of work."""
        ...
        # Count tasks using same pattern as _critique_task_breakdown
        task_pattern = r'^\s*[-*]\s+\[[ x]\]\s+(.+)|^\s*[-*]\s+(.+)'
        matches = re.findall(task_pattern, content, re.MULTILINE)
        task_count = len(matches)

        if self.tier == 'quick' and task_count >= 10:
            self.warnings.append({
                "file": "task-list.md",
                "issue": f"Task list has {task_count} tasks but tier is 'quick'",
                "suggestion": "This may need a standard-spec to properly document requirements and technical design"
            })
        elif self.tier == 'standard' and task_count >= 20:
            self.warnings.append({
                "file": "task-list.md",
                "issue": f"Task list has {task_count} tasks but tier is 'standard'",
                "suggestion": "This may need a full-spec with FRS and Gherkin scenarios for adequate coverage"
            })
        elif self.tier == 'full' and task_count < 5:
            self.warnings.append({
                "file": "task-list.md",
                "issue": f"Task list has only {task_count} tasks but tier is 'full'",
                "suggestion": "This may be over-scoped for a full-spec; consider using 'standard' or 'quick' tier"
            })

    def _critique_requirements(self):
        """Analyze requirement quality in FRD and FRS (tier-aware)."""
        # Check FRD (standard and full tiers)
...
def _parse_args(argv: List[str]) -> Tuple[Path, List[str], str]:
    """Parse CLI arguments.

    Returns (feature_path, focus_areas, tier_or_none).
    """
    if len(argv) < 2:
        return None, [], None
    ...
    # Walk remaining args looking for --focus and --tier
    i = 2
    while i < len(argv):
        if argv[i] == "--focus" and i + 1 < len(argv):
            focus_areas = [area.strip() for area in argv[i + 1].split(',')]
            i += 2
        elif argv[i] == "--tier" and i + 1 < len(argv):
            tier = argv[i + 1].strip().lower()
            if tier not in VALID_TIERS:
                print(json.dumps({
                    ...
                    "critical_issues": [{
                        "file": "N/A",
                        "issue": f"Invalid tier '{tier}'. Must be one of: {', '.join(VALID_TIERS)}",
                        "suggestion": ""
                    }],
                    "warnings": [],
                    "recommendations": []
    ...
    return feature_path, focus_areas, tier


def main():
    """Main entry point."""
    feature_path, focus_areas, tier = _parse_args(sys.argv)
    ...
    if feature_path is None:
        print(json.dumps({
            ...
            "critical_issues": [{
                "file": "N/A",
                "issue": "Usage: python critique_plan.py /path/to/feature-folder [--focus area1,area2] [--tier quick|standard|full]",
                "suggestion": ""
            }],
            "warnings": [],
            "recommendations": []
    ...
    critic = SpecCritic(feature_path, focus_areas, tier)
